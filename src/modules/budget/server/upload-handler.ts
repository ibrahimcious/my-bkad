...
interface UploadConfig<P extends ParsedUpload> {
...
/**
 * Build an uploader-only server function that ingests an `.xlsx` upload.
 *
 * The handler owns the whole pipeline: it gates on the UPLOADER role,
 * reads the FormData file, parses, then runs `persist` and the history
 * write inside one transaction — so a mid-write failure rolls back and
 * leaves the prior dataset intact. A parse failure happens before the
 * transaction opens, so it likewise leaves existing data untouched.
 * Every attempt — success or failure — is recorded in
 * `BudgetUploadHistory` under `config.kind`.
 *
 * Callers supply only what differs: the parser, the persist step, and
 * the labels. This is the single seam every upload pipeline passes
 * through.
 */
export function createUploadFn<P extends ParsedUpload>(
